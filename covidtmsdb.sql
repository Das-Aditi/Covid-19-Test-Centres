...
--
-- Host: 127.0.0.1
-- Generation Time: Mar 03, 2024 at 05:30 PM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdatePatient` (IN `p_ID` INT, IN `p_Name` VARCHAR(120), IN `p_MobileNumber` BIGINT, IN `p_DateOfBirth` DATE, IN `p_GovtIssuedId` VARCHAR(150), IN `p_GovtIssuedIdNo` VARCHAR(150), IN `p_Address` VARCHAR(255), IN `p_State` VARCHAR(200))   BEGIN
    UPDATE `tblpatients`
    SET 
        `FullName` = p_Name,
        `MobileNumber` = p_MobileNumber,
        `DateOfBirth` = p_DateOfBirth,
        `GovtIssuedId` = p_GovtIssuedId,
        `GovtIssuedIdNo` = p_GovtIssuedIdNo,
...
--
-- Triggers `tblpatients`
--
DELIMITER $$
CREATE TRIGGER `tblpatients_insert_audit` AFTER INSERT ON `tblpatients` FOR EACH ROW BEGIN
    INSERT INTO `tblpatients_audit_log` (`FullName`, `MobileNumber`, `DateOfBirth`, `GovtIssuedId`, `GovtIssuedIdNo`, `FullAddress`, `State`, `Action`)
    VALUES (NEW.FullName, NEW.MobileNumber, NEW.DateOfBirth, NEW.GovtIssuedId, NEW.GovtIssuedIdNo, NEW.FullAddress, NEW.State, 'Insert');
END
$$
DELIMITER ;
...
--
-- Table structure for table `tblpatients_audit_log`
--

CREATE TABLE `tblpatients_audit_log` (
  `id` int(11) NOT NULL,
  `FullName` varchar(120) DEFAULT NULL,
  `MobileNumber` bigint(12) DEFAULT NULL,
  `DateOfBirth` date DEFAULT NULL,
...
(8, 785671126, 9901021391, 'Antigen', '2024-03-03T21:56', NULL, NULL, NULL, '2024-03-03 16:24:22', NULL, NULL, NULL);
...
--
-- Indexes for table `tblpatients_audit_log`
--
ALTER TABLE `tblpatients_audit_log`
  ADD PRIMARY KEY (`id`);
...
--
-- AUTO_INCREMENT for table `tblpatients_audit_log`
--
ALTER TABLE `tblpatients_audit_log`
